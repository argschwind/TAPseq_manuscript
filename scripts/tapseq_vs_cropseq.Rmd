...
The number of genome reads in the raw data (fastq) was counted for all samples. Similaril the number
of reads aligned to target genes in the final .bam files was counted. The percentage of all read
mapping to target genes is then compared between conventional CROP-seq and TAP-seq.
...
```{r detectionSensitivity, fig.cap=cap, fig.height=4, fig.width=6}
# add variable for experimantal approach to dge data
dge_filt <- dge_filt %>%
  mutate(approach = if_else(sample == "Sample10X", true = "cropseq", false = "tapseq"))

# calculate number of cells in which a gene is detected
cells_genes <- dge_filt %>%
  group_by(approach, gene) %>%
  summarize(avg_txs = mean(txs),
            cells = sum(txs > 0),
...
expr_quantiles <- cells_genes %>%
  ungroup() %>%
  filter(approach == "cropseq") %>%
  mutate(quantile = ntile(avg_txs, 5)) %>%
...
# save plot to file
ggsave(here("results/plots/detection_sensitivity.pdf"), device = "pdf", height = 4, width = 6)
```

...
**`r snakemake@wildcards$sample`**, for which expression data for
**`r snakemake@config$dge_ncells[snakemake@wildcards$sample]` cells** were extracted.
...
vctr_dge <- dge %>%
  filter(grepl(GENE, pattern = snakemake@params$vector_annot)) %>%
...
`r snakemake@config$perturbation_status$min_txs[snakemake@wildcards$sample]` transcripts per vector
were required to call perturbations.
...
perturb <- perturb %>%
  gather(key = "vector", value = "pert", -cell_barcode)
...
cells_per_pert <- perturb %>%
  ...
  arrange(cells) %>%
  mutate(vector = forcats::fct_inorder(vector))

# plot number of cell per perturbation
ggplot(cells_per_pert, aes(x = vector, y = cells)) +
  geom_bar(stat = "identity") +
  theme_bw() +

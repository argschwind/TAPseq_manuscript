...
# with at least one detected perturbation
...
```{r prepareDGE}
# filter for TAP-seq target genes and convert into one data.frame
dge_filt <- dge %>%
...
Different number of cells are repeatidly drawn (1000 times per size) from the genic reads
downsampled dge data. The LFC between perturbed cells and scrambled control cells is calculated for
every drawn cell sample.
```{r dsFunction}
...
Observed LFC for selected sampling sizes in alternative plotting style:
```{r plotLFCAlt, fig.height=5, fig.width=8}
...
# choose sampling sizes for plot
lfc_stats_plot <- filter(lfc_stats, size %in% c(25, 50, 80)) 

# create plots for chr11 and chr8 panels
plots_chr11 <- lapply(names(known_targets), FUN = function(x){
  plot_lfc(filter(lfc_stats_plot, exp == "chr11"), perturbation = x,
           target_genes = known_targets[[x]])
})

plots_chr8 <- lapply(names(known_targets), FUN = function(x){
  plot_lfc(filter(lfc_stats_plot, exp == "chr8"), perturbation = x,
           target_genes = known_targets[[x]])
})

# print plots
grid.arrange(grobs = plots_chr11, ncol = 2, top = textGrob("Chromosome 11", gp = gpar(fontsize=18)))
grid.arrange(grobs = plots_chr8, ncol = 2, top = textGrob("Chromosome 8", gp = gpar(fontsize=18)))
```

## Percentage negative
The percentage of samples yielding a negative LFC is plotted for selected target genes and across
sampling sizes and methods. The performance seems to be strongly correlated with expression levels
of the genes and TAP-seq provides an advantage for genes with low expression.
```{r plotSig}

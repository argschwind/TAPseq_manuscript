...
plots_chr11 <- lapply(names(known_targets), FUN = function(x){
  plot_lfc(filter(lfc_stats, panel == "chr11"), perturbation = x, target_genes = known_targets[[x]])
})

plots_chr8 <- lapply(names(known_targets), FUN = function(x){
  plot_lfc(filter(lfc_stats, panel == "chr8"), perturbation = x, target_genes = known_targets[[x]])
})
```

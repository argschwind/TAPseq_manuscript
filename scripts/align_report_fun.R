...
plot_mapping_cats <- function(mapping_cats, sample, total_reads = NULL){
  ...
  ggplot(mapping_cats, aes(x = category, y = perc_reads)) +
    geom_bar(stat = "identity") +
    labs(y = "percentage of reads", x = "STAR mapping category",
         title = title) +
...
#' positions where the errors are detected.
#'
...
synthesis_error_hist <- function(synthesis_error_dat, sample,
  ...
  # plot histogram
  ggplot(barcode, aes(x = base, y = fixed_T,
                                       fill = Barcode)) +
    geom_bar(stat = "identity") +
...
#'
#' @param all Logical. If TRUE, all cell barcodes are plotted on the x-axis
#' and \code{nbcs} is used to specified the default view. Turning this option
#' on might result in very large objects!
#'
#' @param title String containing plot title.
#' 
#' @param ncells Number of estimated cells (default: NULL). If specified, a 
#' vertical line will be drawn at this number of cell barcodes.
#'
plot_cumfrac_rpc <- function(cumfrac, nbcs, all_bcs = FALSE, title = NULL, 
                             ncells = NULL){
  
  # only retain data on nbcs cells if all is disabled
  if (all_bcs == FALSE) cumfrac <- cumfrac[1:nbcs, ]
  ...
  p <- plot_ly(cumfrac, x = ~index, y = ~reads_cumfrac, type = "scatter",
               mode = "lines", hoverinfo = "text",
               text = ~paste0("Cells: ", index, "\n",
                              "Reads cell i: ", reads, "\n",
                              "Total reads: ", reads_cumsum, "\n",
                              "Cumfrac reads: ", round(reads_cumfrac,
                                                       digits = 2))
  )
...
#' 
#' Plot reads per cell barcode distribution for all cell barcodes ordered by the
#' number of reads
#' 
#' @param cumfrac A data.frame containing the cumulative fraction of reads per
#' cell barcode calculated by cumfrac_rpc().
...
plot_rpc_distr <- function(cumfrac, title = NULL, ncells = NULL){
  ...
  # create interactive line plot using plotly
  p <- plot_ly(cumfrac, x = ~index, y = ~reads, type = "scatter",
               mode = "lines", hoverinfo = "text",
               text = ~paste0("Cells: ", index, "\n",
                              "Reads cell i: ", reads, "\n",
                              "Total reads: ", reads_cumsum, "\n",
...
#' 
#' Find the local minima between peaks of a multimodal distribution. Works best
#' if applied to the output of a density kernel.
#' 
#' @param x Numeric vector containing the data series in which local minima
#' should be found.
#' 
...
local_minima <- function(x){
  ...
  # get segments where value increases (TRUE) or decreases (FALSE)
  # -Inf at the beginning ensures that the first segment (even if it's only one
  # element) is an increase
...
#' 
#' Apply a density kernel to compute reads per cell barcode distribution. This
#' is expected to result in a bimodal distribution, with real cells and ambient
#' RNA separated by a local mimimum between the two distributions. The function
#' calculates local minima in the distribution and tries to estimate the number
...
#' the second column.
#' 
#' @param expect_cells Integer; A rough estimate of the number of cells
#' contained in the sample.
#' 
#' @return A list containing the computed density (data.frame), reads at local
#' minima, number of used cell barcodes and estimated number of cells.
#' 
rpc_density <- function(read_counts_file, expect_cells){
  ...
  list(density = dens, loc_min_log_reads = loc_min_log_reads,
       loc_min_reads = loc_min_reads, cell_bcs = nbcs, ncells = ncells)
...
#' 
#' @param dens_obj Output from rpc_density() function containing computed
#' density and local minima.
#'
plot_rpc_density <- function(dens_obj, title = NULL){
  ...
  # create plot
  p <- plot_ly(dens, x = ~log10_reads, y = ~density, type = "scatter",
               mode = "lines", hoverinfo = "text",

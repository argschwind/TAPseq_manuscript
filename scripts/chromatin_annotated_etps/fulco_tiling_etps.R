## Create chromatin annotated ETPs for the enhancer tiling experiments in Fulco et al, 2018
...
# Whole-Tx CROP-seq data is used to compute expression in K562 cells.
...
# split gene annotations by gene name
expr_genes <- split(genes, f = genes$gene_name)

# Select enhancers - gene pairs ====================================================================

# Tested enhancers from Fulco et al. 2018 are extracted and ETPs are formed with MYC and GATA1.

# import enhancer sites
enh_coords <- import(here(snakemake@input$enhancers))
...
# create data.frame containing enhancer data
enh_df <- enh_coords %>% 
  data.frame(stringsAsFactors = FALSE) %>% 
  select(perturbation, enh_chr = seqnames, enh_start = start, enh_end = end) %>% 
  mutate(enh_chr = as.character(enh_chr))

# remove enhancers, where liftover moved enhancer to other chromosome
enh_df <- enh_df %>% 
  mutate(chr_id = sub(":.+", "", perturbation)) %>% 
  filter(chr_id == enh_chr) %>% 
  select(-chr_id)

# create data.frame containing gene tss information
tss_df <- gene_tss %>% 
  data.frame(stringsAsFactors = FALSE) %>% 
  select(gene = gene_name, gene_chr = seqnames, gene_start = start, gene_end = end,
         gene_strand = strand) %>% 
  mutate(gene_strand = as.character(gene_strand),
         gene_chr = paste0("chr", as.character(gene_chr)))

# combine enhancers and tss data to form etps
etps <- left_join(enh_df, tss_df, by = c("enh_chr" = "gene_chr")) %>% 
  mutate(gene_chr = enh_chr) %>% 
  select(perturbation, gene, enh_chr, enh_start, enh_end, gene_chr, gene_tss = gene_start,
         gene_strand)

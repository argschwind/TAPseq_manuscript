## Create chromatin annotated ETPs for the entire genome
...
# Select enhancers - target pairs ==================================================================

# Predicted enhancer sites are selected that are within 300kb of the expressed genes TSS

# load genome-wide enhancers
enh <- read.table(here(snakemake@input$enhancers), col.names = "enh_id", stringsAsFactors = FALSE)

# split enhancer id into chromosome, start and end coordinates
enh <- enh %>% 
  separate(col = enh_id, into = c("chr", "start", "end"), convert = TRUE, remove = FALSE) %>% 
  mutate(chr = sub("chr", "", chr))

# create GRanges object containing enhancer coordinates
enh_coords <- makeGRangesFromDataFrame(enh, keep.extra.columns = TRUE)
...
etps <- data.frame(perturbation = enh_df$enh_id, gene = tss_df$gene_name,
...
etps_chrs <- etps_chrs[names(hic_data)]

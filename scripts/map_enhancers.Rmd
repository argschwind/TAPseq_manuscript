...
p1 <- ggplot(cells_per_pert, aes(x = sample, y = cells, fill = sample)) +
  geom_violin() +
  labs(y = "Cells", title = "Cells per perturbation") +
  scale_fill_manual(values = c("Chr11" = "indianred3", "Chr8" = "steelblue")) +
...
ggsave(p1, filename = here("results/plots", "cells_per_pert.pdf"), device = "pdf", width = 5, 
       height = 5)
```

```{r pertsPerCell, fig.width=10, fig.height=5}
...
p2 <- ggplot(perts_per_cell_hist, aes(x = perts, y = cells / 1000, fill = sample)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Enhancer perturbations per cell", y = "Cells (x 1000)",
       fill = "Region:") +
  scale_fill_manual(values = c("Chr11" = "indianred3", "Chr8" = "steelblue")) +
  theme_bw() +
  theme(text = element_text(size = 20), legend.position = "top", panel.grid = element_blank())

# save plot for manuscript
ggsave(p2, filename = here("results/plots", "perts_per_cell.pdf"), device = "pdf", width = 5, 
       height = 5)
...
The number of cis-enhancers per gene are counted and plotted.

```{r enhPerGene, fig.width=5, fig.height=4.5}
...
ggplot(enh_counts, aes(x = factor(enh, levels = seq_len(max(enh))), y = genes, fill = sample)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Enhancers", y = "Genes", title = "Enhancers per gene",
       fill = "Region") +
  scale_fill_manual(values = c("Chr11" = "indianred3", "Chr8" = "steelblue")) +
  scale_x_discrete(drop = FALSE) +
  theme_bw() +
  theme(text = element_text(size = 20), legend.position = c(0.84, 0.82),
        panel.grid = element_blank())

# save plot for manuscript
ggsave(here("results/plots", "enhancers_per_gene.pdf"), device = "pdf", width = 5, height = 4.5)
```
...
### Nearest gene
Every significant pair is assessed whether the target is the closest gene to the enhancer.

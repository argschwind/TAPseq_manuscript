...
mkdir -p ../data/TAP_DEEP_p1/downsampled
wget http://steinmetzlab.embl.de/TAPdata/TAP_DEEP_p1.zip
unzip -j -d ../data/TAP_DEEP_p1/downsampled TAP_DEEP_p1.zip

mkdir -p ../data/WTX_REF_Deep/downsampled
wget http://steinmetzlab.embl.de/TAPdata/WTX_REF_DEEP.zip
unzip -j -d ../data/WTX_REF_Deep/downsampled WTX_REF_DEEP.zip

rm TAP_DEEP_p1.zip WTX_REF_DEEP.zip
```
...
folders <- c(
    "k562.82.10x.low" = "../data/TAP_DEEP_p1/downsampled/"
)
...
getUMI2 <- function(ps, frPositive=T, onPanel=T) {
  folder <- "../data/WTX_REF_Deep/downsampled/"
  reads <- as.integer(unique(gsub("dge_(.+)_avg_reads_per_cell.txt","\\1",list.files(folder,"dge_.+_avg_reads_per_cell.txt"))))
...
#this corresponds to ../data/WTX_REF_Deep/dge.txt if the snakemake pipeline was run.

...
mkdir -p data
wget http://steinmetzlab.embl.de/TAPdata/re11iv210ng.zip
unzip -j -d data/re11iv210ng re11iv210ng.zip

wget http://steinmetzlab.embl.de/TAPdata/Sample10X.zip
unzip -j -d data/Sample10X Sample10X.zip

rm re11iv210ng.zip Sample10X.zip
```
...
knowns <- c("HBG1","HBG2","HBE1","HBD","HBB","GATA1","ZFPM2")
...
folders <- c(
    "k562.82.10x.low" = "data/re11iv210ng/"
)
...
ggplot(aes(x = reads, y = UMIperCell, color = onPanel, group = paste(experiment, onPanel)), data= UMIstat) + stat_summary(mapping = aes(linetype = experiment), fun.y = mean, geom = "line") + 
  scale_y_log10() + scale_color_manual(values = c("FALSE" = "blue", "TRUE" = "red"),labels = c("FALSE" = "Genome", "TRUE" = "Target panel"),name = "View") + scale_linetype_discrete(labels = c("k562.82.10x.low" = "TASC-seq", "k562.whole.10x" = "Whole Tx"), name = "Method") +
  theme_bw() + xlab("Average reads per cell") + ylab("Average UMI observed") + scale_x_log10(limits = c(1000,20000), breaks = c(1000,3000,8000,20000))#scale_x_continuous(limits = c(1000,20000), breaks = seq(1000,20000,by=2000), labels = seq(1,20,by=2))
...
```{r panele, fig.width=5,fig.height=4}
 ggplot(aes(x = reads, y = UMIperCell, color = onPanel, group = paste(experiment, onPanel)), data= UMIstat) + stat_summary(mapping = aes(linetype = experiment), fun.y = mean, geom = "line") + 
  scale_y_log10() + scale_color_manual(values = c("FALSE" = "blue", "TRUE" = "red"),labels = c("FALSE" = "Genome", "TRUE" = "Target panel"),name = "View") + scale_linetype_discrete(labels = c("k562.82.10x.low" = "TASC-seq", "k562.whole.10x" = "Whole Tx"), name = "Method") +
  theme_bw() + xlab("Average reads per cell") + ylab("Average UMI observed") + scale_x_log10(limits = c(1000,20000), breaks = c(1000,3000,8000,20000))#scale_x_continuous(limits = c(1000,20000), breaks = seq(1000,20000,by=2000), labels = seq(1,20,by=2))
...
total.nods <- readRDS(url("http://steinmetzlab.embl.de/TAPdata/k562.whole.10x.RDS"))
...
ggplot(aes(x = reads, y = 100*frPositive), data= frPositive) + stat_summary(aes(color =as.factor(bin), linetype = experiment, group= paste(experiment, bin) ), geom="line", fun.y=median) + 
  scale_linetype_discrete(labels = c("k562.82.10x.low" = "TAP-seq", "k562.whole.10x" = "Whole Tx"),name = "Method") + theme_bw() + xlab("Average reads per cell") + ylab("Average UMI observed") + scale_x_log10() +
   ylab("% of cells positive") + scale_color_manual(values = c("1" = "black","2"="blue","3"="red","4"="orange"), name = "Gene expression\nlevel") + scale_x_log10(limits = c(1000,20000), breaks = c(1000,3000,8000,20000))#scale_x_continuous(limits = c(1000,20000), breaks = seq(1000,20000,by=2000), labels = seq(1,20,by=2))
